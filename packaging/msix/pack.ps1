<#
  Build + sign the loop-slice MSIX from an already-exported Win64 Godot build.
  Requires Windows + Windows SDK (makeappx.exe, signtool.exe).

  -BinDir   folder with loop-slice.exe + loop-slice.pck (the Godot Windows export)
  -Version  4-part version, e.g. 0.1.0.0
  ...
  ex: pwsh packaging/msix/pack.ps1 -BinDir build/windows -Version 0.1.0.0
#>
[CmdletBinding()]
param(
...
$root = Join-Path ([System.IO.Path]::GetTempPath()) ("loop-slice-msix-" + [guid]::NewGuid())
...
$msix = Join-Path $OutDir "v-sekai-loop-slice-$Version.msix"
...
if (-not $PfxPath) {
  ...
  $PfxPath = Join-Path $OutDir "loop-slice-test.pfx"; $PfxPassword = "test"
